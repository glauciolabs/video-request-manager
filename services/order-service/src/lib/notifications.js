...
const NOTIFICATION_SERVICE_URL_RAW = process.env.NOTIFICATION_SERVICE_URL || 'http://notification-service:3003/notifications';
const NOTIFICATION_SERVICE_URL = (() => {
  const trimmed = String(NOTIFICATION_SERVICE_URL_RAW).replace(/\/+$/, '');
  return trimmed.endsWith('/notifications') ? trimmed : `${trimmed}/notifications`;
})();
const ADMIN_NOTIFICATION_EMAILS = Array.from(
  new Set(
...
const PIX_PAYMENT_KEY = String(process.env.PIX_PAYMENT_KEY || '').trim();
...
function getCustomerNotificationIntro(type, order) {
  ...
  const byType = {
    new_order: `Olá, ${requester}. Seu pedido foi recebido e está aguardando pagamento para início da produção.`,
...
function getAdminNotificationIntro(type, order) {
  const statusLabel = STATUS_LABELS[order.status] || order.status;
  const byType = {
    new_order: 'Novo pedido recebido e aguardando confirmação de pagamento.',
...
function buildOrderRows(order) {
  ...
  const assetsConfirmation = order.assetsReadyConfirmed
    ? (
      order.requestType === 'add_photos'
        ? 'Confirmado: arquivos de adição + vídeo processado anterior presentes na pasta de entrada'
        : 'Confirmado: todos os arquivos do novo pedido estão na pasta de entrada'
    )
    : 'Não confirmado';
  ...
  return [
    ...
    ['IP do solicitante', order.requestIp || '-'],
    ['Tipo de solicitação', REQUEST_TYPE_LABELS[order.requestType] || order.requestType || '-'],
    ['Checklist de arquivos de entrada', assetsConfirmation],
    ...
    ['Chave PIX para pagamento', PIX_PAYMENT_KEY || '-'],
...
function buildNotificationContent(type, order, customMessage) {
  ...
  const adminText = buildAdminTelegramBody(type, order, customMessage);
  const reference = order.orderNumber || order.id;

  return {
    subject: `${title} #${reference}`,
    adminSubject: `VRM Admin | ${title} #${reference}`,
    text: customerText,
