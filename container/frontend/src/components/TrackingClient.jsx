...
function getAssetsChecklistLabel(order, t) {
  if (!order?.assetsReadyConfirmed) return t('tracking.assetsChecklistNo');
  return order.requestType === 'add_photos'
    ? t('tracking.assetsChecklistYesAdd')
    : t('tracking.assetsChecklistYesNew');
}
    ...
    <section className="form-shell">
      ...
      <form className="request-form" onSubmit={onSubmit}>
        ...
        <label className="field">
          ...
          <input
            ...
            placeholder="Código de rastreio (8 caracteres)"
            required
          />
        </label>
      ...
      {result && (
        <article className="tracking-result">
          ...
          <p><strong>{t('tracking.assetsChecklist')}:</strong> {getAssetsChecklistLabel(result, t)}</p>
