...
### Pull de imagem no cluster (quando GHCR privado)
Crie `ghcr-pull-secret` nos namespaces de destino:
```bash
kubectl -n video-request-manager-develop create secret docker-registry ghcr-pull-secret \
  --docker-server=ghcr.io \
  --docker-username='glauciocampos' \
  --docker-password='<github-pat-com-read-packages>' \
...
`--docker-username` deve ser o usuário dono do PAT (não a organização).
...
5. Garantir existência do secret `ghcr-pull-secret` nos namespaces de deploy.
